...
void setup()
{
  ...
  SPI.begin () ;
  //--- Configure ACAN2515
  ACAN2515Settings settings (QUARTZ_FREQUENCY, 500UL * 1000UL) ; // CAN bit rate 125 kb/s, а как будто 500 kb/s???
...
void loop()
{
  ...
  if (can.available ()) {
    ...
    else if (frame.data[2] == 0x0E) // если джойстик выдает 0E значит он в нейтральном положении, в положении DS он постоянно выдает 7E
    {
      ...
      DS_trans = 0;
      // если нет сообщений, то обнуляем все переменные
      long2self = 0;
      short2self = 0;
      longFromSelf = 0;
      shortFromSelf = 0;
  ...
  if (short2self) {
    ...
    if (inhibitorN && !blockSpeed) { // если игибитор в N то контролируем скорость для переключения в D? НАДО ЛИ ?
      analogWrite(mot1, power); // запускаем моторчик прямо, т.е. от P->D
      moving = 1; // выставляем флаг работы моторчика
      moving2D = 1; // выставляем флаг работы моторчика до положения D
    }
